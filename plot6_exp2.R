...
NEI_Baltimore_LosAngeles <- subset(NEI, fips %in% c("24510","06037"))
...
merged_data <- merge(NEI_Baltimore_LosAngeles, SCC_VEHICLES , by = "SCC")

## Remove variables to free space
remove(SCC, SCC_VEHICLES, NEI_Baltimore_LosAngeles)

## Calculate total values for each year
tot_emissions <-  ddply(merged_data, .(fips,year), summarize, totEmissions = sum(Emissions) )
...
## Generate Plot
## Add proper labels
Country <- data.frame(fips=c("06037","24510"),Country=c("Los Angeles","Baltimore"))
tot_emissions2 <- merge(tot_emissions,Country, by = "fips")

g <- ggplot(data = tot_emissions2, aes(year,totEmissions))+geom_point(aes(color=Country), size=4)+geom_smooth(method="lm", se=FALSE, mapping = aes(color=Country))+labs(x="Year")+labs(y="Total Emissions [ tons ]")+labs(title=expression("Total "* PM[2.5]* " Emissions from Vehicles"))
print(g)
